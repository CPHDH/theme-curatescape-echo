...
if($collection && ($file=$collection->getFile())){
    ...
    $serverpath = FILES_DIR.DIRECTORY_SEPARATOR.$s.DIRECTORY_SEPARATOR.$filename;
    $size = getimagesize($serverpath);
    ...
    array_push($image, array(
        ...
        'caption'=>'<span class="file-title" itemprop="name">'.link_to('items', 'browse', __("Browse %1s %2s in this collection.", metadata('collection', 'total_items'), rl_item_label(true)), array('itemprop'=>'contentUrl','class'=>''), array('collection'=>$collection->id)).'</span>',));
}else{
    $hero_img = null;
}
...
<article class="browse collection" id="content" role="main">
    ...
    <?php echo '<div class="byline">'.rl_icon('folder').'&nbsp;'.__('%1s %2s', metadata($collection, 'total_items'), rl_item_label(true)).'</div>'; ?><br>
    <div id="primary" class="browse">
        <section id="text">
            <div id="collection-description">
                ...
                <?php echo link_to('items', 'browse', __("Browse %1s %2s in this collection.", metadata('collection', 'total_items'), rl_item_label(true)), array('class'=>'button collection-items-browse'), array('collection'=>$collection->id)); ?>
                <?php else: ?>
                <br><br>
                <p><?php echo __("This collection currently has no %s.", rl_item_label(true)); ?></p>
                <?php endif; ?>
            </div>
        </section>
