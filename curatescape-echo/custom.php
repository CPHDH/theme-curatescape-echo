...
function rl_search_record_types($recordTypes)
{
    ...
    if(plugin_is_active('TourBuilder','1.6','>=')) $recordTypes['Tour'] = rl_tour_label('singular');
    return $recordTypes;
}
...
?>  
	<nav id="top-navigation" class="" aria-label="<?php echo __('Main Navigation');?>">
		...
		<div id="nav-interactive">
			<!-- Search -->
			<a tabindex="0" title="<?php echo __('Search');?>" id="search-button" href="#footer-search-form" class="button transparent"><?php echo rl_icon("search");?><span><?php echo __('Search');?></span></a>
			<!-- Menu Button -->
			<a tabindex="0" title="<?php echo __('Menu');?>" id="menu-button" href="#footer-nav" class="button transparent"><?php echo rl_icon("menu");?><span><?php echo __('Menu');?></span></a>
		</div>
...
function rl_story_map_single($title=null,$location=null,$address=null,$hero_img=null,$hero_orientation=null){ ?>
	...
	<figure id="story-map"
	data-default-layer ="<?php echo get_theme_option('map_style') ? get_theme_option('map_style') : 'CARTO_VOYAGER';?>"
	data-lat="<?php echo $location[ 'latitude' ];?>" 
	data-lon="<?php echo $location[ 'longitude' ];?>" 
	data-zoom="<?php echo $location['zoom_level'];?>" 
	data-title="<?php echo strip_tags($title);?>" 
	data-image="<?php echo $hero_img;?>" 
	data-orientation="<?php echo $hero_orientation;?>"
	data-address="<?php echo strip_tags($address);?>" 
	data-color="<?php echo get_theme_option('marker_color');?>"
	data-featured-color="<?php echo get_theme_option('featured_marker_color');?>"
	data-featured-star="<?php echo get_theme_option('featured_marker_star');?>"
	data-rool-url="<?php echo WEB_ROOT;?>"
	data-maki-js="<?php echo src('maki/maki.min.js','javascripts');?>"
	data-providers="<?php echo src('providers.js','javascripts');?>"
	data-leaflet-js="<?php echo src('theme-leaflet/leaflet.js','javascripts');?>"
	data-leaflet-css="<?php echo src('theme-leaflet/leaflet.css','javascripts');?>"
	>
		<div class="curatescape-map">
			<div id="curatescape-map-canvas"></div>
		</div>
	...
	<div id="map-actions"><a class="button directions" target="_blank" rel="noopener" href="https://maps.google.com/maps?location&daddr=<?php echo $address ? urlencode(strip_tags($address)) : $location[ 'latitude' ].','.$location[ 'longitude' ];?>"><?php echo rl_icon("logo-google",null);?><span class="label"><?php echo __('Open in Google Maps');?></span></a></div>
<?php }
...
function rl_subjects($raw=false, $rawfirst=false){
	...
	if (count($subjects) > 0){
		foreach ($subjects as $subject){
			...
			$link .= htmlentities('&search=&advanced[0][element_id]=49&advanced[0][type]=contains&advanced[0][terms]=');
			$link .= urlencode(str_replace('&amp;','&',$subject));
			$node = '<li><a title="'.__('Subject').': '.$subject.'" class="tag tag-alt" href="'.$link.'">'.$subject.'</a></li>';
...
function rl_meta_style($heading=null,$array=array()){
	...
	if($html){
		return '<div class="meta-'.str_replace(' ','-',strtolower($heading)).'"><h3 class="metadata-label">'.$heading.'</h3><div class="meta-style">'.$html.'</div></div>';
	}else{
		return null;
	}
}
...
function rl_map_caption($item='item'){
	$caption=array();
	if($addr=rl_street_address($item)) $caption[]=strip_tags($addr,'<a>');
	if($accs=rl_access_information($item,false)) $caption[]=strip_tags($accs,'<a>');
...
function rl_streaming_files($filesArray=null,$type=null,$openFirst=false){
	...
	foreach ($filesArray as $file){
		...
		$html.='<div>';
			...
			if($type == 'audio'){
				$html.='<audio controls preload="auto"><p class="media-no-support">'.__('Your web browser does not support HTML5 audio').'</p><source src="'.WEB_ROOT.'/files/original/'.$file['src'].'" type="audio/mp3"></audio>';
			}elseif($type="video"){
				$html.='<video playsinline controls preload="auto"><source src="'.WEB_ROOT.'/files/original/'.$file['src'].'" type="video/mp4"><p class="media-no-support">'.__('Your web browser does not support HTML5 video').'</p></video>';
			}		
			$html .='</div>';
...
function rl_google_analytics(){
	...
	if (substr($id,0,2) == 'G-'): ?>
		...
		<script>
		  ...
		  gtag('js', new Date());
		  gtag('config', '<?php echo $id;?>', {cookie_flags: 'SameSite=None;Secure'});
		</script>
