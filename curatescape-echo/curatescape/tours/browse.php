...
if (isset($_GET['tags'])) {
   $title = __('%1$s tagged "%2$s": %3$s', rl_tour_label(true), htmlspecialchars($_GET['tags']),'<span class="item-number">'.$total.'</span>');
   $title_facet = ' | '.htmlspecialchars($_GET['tags']);
   $scroll_to = 'content';
}elseif (isset($_GET['featured']) && $_GET['featured'] == 1) {
    $title = __('Featured %1$s: %2$s', $label, '<span class="item-number">'.$total.'</span>');
    $title_facet = ' | '.__('Featured');
    $scroll_to = null;
} else {
    $title = __('All %1$s: %2$s', $label, '<span class="item-number">'.$total.'</span>');
    $title_facet = null;
    $scroll_to = null;
}
   ...
   <article class="browse tour">
      <div class="browse-header">
         ...
         <div id="helper-links">
            ...
            &nbsp;&nbsp;
            <span class="helper-label"><?php echo rl_icon('information-circle').'&nbsp;'.(get_theme_option('tour_info') ? strip_tags(get_theme_option('tour_info'),'<a>') :  __("%s are self-guided.", rl_tour_label(true))); ?></span>
         </div>
      </div>
      <div id="primary" class="browse">
      <section id="results" aria-label="<?php echo rl_tour_label(false);?>">
         ...
         if ($total > 0) {
            if(function_exists('activeSort')) {
               ...
               $tours = activeSort($tours, $active_sort);
            }
            foreach ($tours as $tour) {
               ...
               $html .= '<p class="tour-snip">'.snippet(strip_tags(htmlspecialchars_decode(tour('description'))), 0, 200).'<br><a class="readmore" href="'.WEB_ROOT . '/tours/show/'. tour('id').'">'.__('View %s', rl_tour_label(false)).'</a></p>';
               $html .= '</div>';
               $html .= '</article>';
            }
         } else {
